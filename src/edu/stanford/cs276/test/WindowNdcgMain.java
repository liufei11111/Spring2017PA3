...
public class WindowNdcgMain {
  ...
  private static Map<Query,List<Pair<Document,Double>>> score(
    ...
    if (scoreType.equals("baseline")) {
      ...
      SmallestWindowScorer temp = (SmallestWindowScorer) scorer;
      temp.B = additionalConfig.get(0).getSecond();
  ...
  public static void main(String[] args) throws IOException {
    ...
    WindowNdcgMain test = new WindowNdcgMain();
...
//    writeRankedResultsToFile(queryRankings,outputFilePath);
    ...
    additionalParams.add(new Pair("B",new Pair(1.0,2.0)));
    Double[] additionalArrays = {2.0};
...
//        //
  ...
  public Pair<List<Pair<Pair<Integer,Integer>, Double>>,Double> linearConstrainTunner(List<Pair<String,Pair<Double,Double>>> parameters,
      ...
      Map<String, Double> idfs,WindowNdcgMain test, Map<String,Map<String,Double>> relevScores, List<Pair<Pair<Integer,Integer>, Double>> additionalConfig){
...
//        tunner.printConfig(config);
        ...
        Map<Query,List<Pair<Document,Double>>> queryRankings = score(queryDict, taskOption, idfs, null, additionalConfig);
...
//    System.out.println("Best Score: "+bestScore);
    List<Pair<Pair<Integer,Integer>, Double>> list = new ArrayList<>();
        list.add(new Pair(new Pair(1,1),bestScore));
    return new Pair(list,bestScore);
  }
